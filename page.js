$("document").ready(function() {
	...
	$.get("loaddata.php?action=row_count", function(data) {
		...
		$("#page_count").val(Math.ceil(data / 3));
		generateRows(1);
		//generisi miseve sa prve strane
	});
});
...
function generateRows(selected) {
	...
	if (pages <= 5) {
		...
		$("#paginator").append(
			"<a href='#content' class='pagor'>1</a>" +
			"<a href='#content' class='pagor'>2</a>" +
			"<a href='#content' class='pagor'>3</a>" +
			"<a href='#content' class='pagor'>4</a>" +
			"<a href='#content' class='pagor'>5</a>"
		);
